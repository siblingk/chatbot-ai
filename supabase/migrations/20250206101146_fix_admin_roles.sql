...
DROP FUNCTION IF EXISTS public.set_user_role(UUID, auth.user_role);
DROP FUNCTION IF EXISTS public.get_users_with_roles();
DROP FUNCTION IF EXISTS public.ensure_initial_admin();

-- We already have the user_role enum, so we'll skip its creation
-- and just focus on adding the column if it doesn't exist
DO $$  
BEGIN     
    IF NOT EXISTS (         
        SELECT 1          
